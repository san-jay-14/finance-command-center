...
- log_transaction: the user is reporting a transaction they have ALREADY completed — a past-tense statement of fact ("I bought 10 TCS at 3800", "I sold 5 grams of gold") — and gave enough detail to log it (asset class, action, and quantity at minimum). This tool only records completed transactions; it does NOT place broker orders. So an imperative command to trade NOW ("buy 10 shares of TCS", "sell all my Reliance at market") is NOT a log_transaction — this app cannot execute live orders, so use ask_clarification for those instead of silently logging them. Estimate "amount" (total transaction value) from what they said if possible. For gold/real_estate/other/mutual_fund, include asset_name — a short label for what was bought (e.g. "flat", "car", "Gold"). For mutual funds, include scheme_code if the user gives one (an MFAPI.in scheme code); if they don't, omit it — a NAV can't be looked up without it.
